...
print("=== RAW TELEMETRY: Checking container status ===")
subprocess.run(["sudo", "docker", "ps", "-a", "--filter", f"id={container_id}", "--format", "{{.Status}}"], check=True)

print("\n=== STEP 1: RESTORE the original working App.jsx ===")
# We are going to write a HIGHLY SIMPLIFIED version that ALWAYS works.
# It will NOT have the complex rendering, but it WILL be valid and build.
# We will FIX the data flow in the API next.
simple_app_jsx = """import React, { useState, useEffect } from 'react'
...
with open(host_file, 'w') as f:
    f.write(simple_app_jsx)
print("✅ Simple working App.jsx written.")

print("\n=== STEP 2: Copy to container and restart ===")
...
print("\n=== RAW TELEMETRY: Verify build (No errors) ===")
build_result = subprocess.run(
    ["sudo", "docker", "exec", container_id, "sh", "-c", "cd /app && npx vite build 2>&1 | head -n 10"],
    capture_output=True, text=True
)
print(build_result.stdout)

print("\n=== RAW TELEMETRY: Verify API (Check real_info inside docker_containers) ===")
curl_result = subprocess.run(["curl", "-s", "http://localhost:3000/api/forensic-data"], capture_output=True, text=True)
try:
    ...
    if target:
        real_info = target.get('real_info', {})
        mounts = real_info.get('mounts', [])
        print(f"Container: {target['name']}")
